...
public interface ISkillExperienceDao extends JpaRepository<SkillExperience, Long> {
    ...
    @Query("select se from SkillExperience se where se.jobSeeker.id = :id order by se.id")
    List<SkillExperience> findByJobSeekerId(@Param("id") Long jobSeekerId);

    List<SkillExperience> findByJobSeeker(JobSeeker jobSeeker);
