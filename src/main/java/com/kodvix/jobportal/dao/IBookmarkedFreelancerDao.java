...
public interface IBookmarkedFreelancerDao extends JpaRepository<BookmarkedFreelancer, Long> {
	...
	@Query(value="select new com.org.kodvix.freelanceapp.dto.BookmarkedFreelancerListDTO(bf.id, CONCAT(bf.freelancer.firstName, ' ', bf.freelancer.lastName) as freelancerName, bf.freelancer.userName, CONCAT(bf.bookmarkedBy.firstName, ' ', bf.bookmarkedBy.lastName) as recruiterName, bf.bookmarkedBy.userName) from BookmarkedFreelancer bf order by bf.id")
